...
public final class DefaultHBaseDDLExecutor implements HBaseDDLExecutor {
  ...
  @Override
  public void createTableIfNotExists(HTableDescriptor descriptor, @Nullable byte[][] splitKeys)
    throws IOException {
    try (HBaseAdmin admin = new HBaseAdmin(hConf)) {
      ...
      try {
        LOG.debug("Attempting to create table '{}' if it does not exist", Bytes.toString(descriptor.getName()));
        admin.createTable(descriptor, splitKeys);
        LOG.info("Table created '{}'", Bytes.toString(descriptor.getName()));
        ...
        LOG.debug("Table '{}' already exists.", Bytes.toString(descriptor.getName()), e);
      }
        ...
        do {
          if (admin.tableExists(descriptor.getName())) {
            LOG.info("Table '{}' exists now. Assuming that another process concurrently created it.",
                     Bytes.toString(descriptor.getName()));
            return;
          } else {
      ...
      LOG.error("Table '{}' does not exist after waiting {} ms. Giving up.", Bytes.toString(descriptor.getName()),
  ...
  @Override
  public void modifyTable(String namespace, String name, HTableDescriptor descriptor)
