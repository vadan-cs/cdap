...
class ImpersonatingDatasetAdmin implements DatasetAdmin {
  ...
  ImpersonatingDatasetAdmin(DatasetAdmin delegate, Impersonator impersonator, NamespacedEntityId entityId) {
  ...
  private <T> T execute(final Callable<T> callable) throws IOException {
    try {
      return impersonator.doAs(entityId, callable);
      ...
      LOG.warn("Unexpected exception while executing dataset admin operation in namespace {}.", entityId, t);
